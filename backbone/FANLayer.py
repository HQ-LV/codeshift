...
class FANLayer(nn.Module):
    ...
    def init_linear_weights(self, linear_layer):
        """
        自定义nn.Linear层的初始参数
        """
        ...
        linear_layer.weight = nn.Parameter(
            (1 / in_features) * torch.ones([out_features, in_features])
        )
    def init_fanlayer_weights(self): 
        self.init_linear_weights(self.input_linear_p)
        self.init_linear_weights(self.input_linear_g) 
