...
def meta_weight_net(): 
    ...
    for epoch in range(args.max_epoch):
        ...
        all_labels = []
        all_outputs = []
        for iteration, (inputs, labels,_) in enumerate(main_dataloader):
            ...
            # Store the inputs, labels, and outputs for this iteration
            all_inputs.append(inputs.cpu().detach() )
        ...
        if epoch%20==0:
            plot_compare_sig(labels,outputs, input=inputs,title='train')
        ...
        if val_loss < best_val_loss:
            ...
            torch.save(net.state_dict(), best_model_path+f'/best_model.pth')
            print(f"Best model saved at epoch {epoch} with val_loss {val_loss:.4f}")

        # After every 20 epochs, save the data
        if epoch % 20 == 0:
            epoch_save_path = os.path.join(best_model_path, f"output_{epoch}.pt")
            # Save inputs, labels, and outputs for the entire epoch
            torch.save({
            ...
            torch.save(net.state_dict(), best_model_path+f'/model_{epoch}.pth')
...
def plot_compare_sig(labels, recons,logvar=None,input = None,title = 'val'):
    # 随机选择一个样本的索引
    ...
    plt.plot(labels[sample_index, 0, :].detach().cpu().numpy(), label='True', color='blue')
    plt.plot(recons[sample_index, 0, :].detach().cpu().numpy(), label='Recons', color='orange')
